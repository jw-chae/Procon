#!/usr/bin/env python
"""ProCon: depth-selective soft-projection consensus for training-free UAD.

ProCon builds independent coreset memories for selected DINOv2 layers, computes
soft-projection residual maps, and combines them with fixed consensus operators.
No encoder or decoder is trained.
...
Ablation baselines (sections 6.A-6.C):
    python run_procon.py --dataset mvtec --method single
    python run_procon.py --dataset mvtec --method full
    python run_procon.py --dataset mvtec --method random
"""
...
import procon.data.datasets  # noqa: F401
from procon.models.feature_extractors import vit_patchcore  # noqa: F401
from procon.models.backbones import dinov2_multilayer  # noqa: F401
from procon.inference import torch_knn, faiss_gpu  # noqa: F401

from procon.consensus.recipes import get_recipe, list_recipes
from procon.consensus.runner import (
    evaluate_category,
    evaluate_category_layergroup,
)
from procon.engine import build_feature_extractor
from procon.utils import get_dataset_categories, load_yaml, save_json
...
VISA_DEFAULT_ROOT = "datasets/visa"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ProCon training-free UAD")
...
def main() -> None:
    ...
    print(f"\n{'='*64}")
    print(f"  ProCon | dataset={args.dataset} | recipe={args.recipe} | tag={tag}")
    ...
    out_dir = Path(args.output) if args.output else Path("runs_procon") / args.dataset / tag
